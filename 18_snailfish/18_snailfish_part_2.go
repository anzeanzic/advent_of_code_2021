...
func ReadFile() {
	...
	for i := 0; i < len(snailfish_numbers); i++ {
		for j := 0; j < len(snailfish_numbers); j++ {
			if i == j {
				continue
			}

			snailfish_number_combo := make([][]string, 0)
			snailfish_number_combo = append(snailfish_number_combo, snailfish_numbers[i])
			snailfish_number_combo = append(snailfish_number_combo, snailfish_numbers[j])

			magnitude := Calculate(snailfish_number_combo)
	...
	fmt.Println("Largest magnitude", largestMagnitude)

	//CheckTheMagnitude(snailfish_numbers[0])
}

func Calculate(snailfish_numbers [][]string) int {
	last_added_ndx := 0
	...
	magnitude := CheckTheMagnitude(snailfish_numbers[0])
