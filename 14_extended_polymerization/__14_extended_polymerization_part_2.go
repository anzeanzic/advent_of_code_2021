...
func ReadFile() {
	...
	for i, _ := range template {
		if i < len(template) {
			...
			if _, ok := adjacent_map[string(current_rune)]; ok {
				adjacent_map[string(current_rune)]++
			} else {
				adjacent_map[string(current_rune)] = 1
			}
		}
	}
...
func AsStepsGoBy(adjacent_map map[string]int64, template string, part_insertion_rules []part_rule) string {
	for str1, _ := range adjacent_map {
		for str2, _ := range adjacent_map {
			adjacent_runes := str1 + str2

			inserted := CheckIfAnyRuleApplies(part_insertion_rules, adjacent_runes)

			if len(inserted) > 0 {
				if _, ok := adjacent_map[inserted]; ok {
					adjacent_map[inserted] += 1
				} else {
					adjacent_map[inserted] = 1
				}
			}
